const journeyData = [
  {
    ...
    date: "2022 - 2025",
    desc: "Bachelor of Arts from one of India's most prestigious universities. Built the foundation for analytical thinking and creative problem-solving.",
    badges: ["Education", "BA Graduate"],
    color: "#A663CC" // purple
  },
  {
    title: "PUMA",
    date: "2021 - 2023",
    desc: "Floor Manager leading retail operations and team coordination. Recognized for exceptional performance with 7× awards for sales and leadership.",
    badges: ["Floor Manager", "7× Award Winner"],
    color: "#EC4899" // pink
  },
  {
    title: "AKS Valuers",
    date: "2023 - Present",
    desc: "IT & Business Manager architecting automation systems and digital infrastructure. Earned 'Innovation IT Architect' certification.",
    badges: ["IT Manager", "IT Architect Certified"],
    color: "#3B82F6" // blue
  },
  {
    title: "Formulaic Engineers",
    date: "Present",
    desc: "Manager Automation & IT, building PAN India systems spanning 80+ offices. Leading digital transformation through custom automation solutions.",
    badges: ["Current Role", "80+ Offices"],
    color: "#22C55E" // green
  }
];
...
class CardStreamController {
  ...
  animate() {
    ...
    if (this.isAnimating && !this.isDragging) {
      ...
      this.updateCardPosition();
      if (this.speedIndicator) this.updateSpeedIndicator();
    }
  ...
  toggleAnimation() {
    ...
    const btn = document.querySelector(".journey-control-btn");
  ...
  onWheel(e) {
    if (!e.target.closest('#journey')) return;

    // allow lenis to scroll the page vertically but also scroll cards
    const scrollSpeed = 5;
  ...
  createCardWrapper(index) {
    ...
    wrapper.className = "journey-card-wrapper";

    const normalCard = document.createElement("div");
    normalCard.className = "journey-card journey-card-normal";

    const data = journeyData[index % journeyData.length];

    normalCard.style.background = `linear-gradient(135deg, #111, #000)`;
    normalCard.style.border = `1px solid ${data.color}40`;

    normalCard.innerHTML = `
        <div style="padding: 30px; height: 100%; display: flex; flex-direction: column; justify-content: space-between; position: relative; z-index: 10;">
            <div>
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                    <h3 style="font-size: 24px; font-weight: bold; margin: 0; color: white;">${data.title}</h3>
                    <span style="font-family: monospace; font-size: 12px; color: ${data.color}; background: ${data.color}20; padding: 4px 10px; border-radius: 20px;">${data.date}</span>
                </div>
                <p style="color: #aaa; font-size: 14px; line-height: 1.5; margin: 0; white-space: normal;">${data.desc}</p>
            </div>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                ${data.badges.map(b => `<span style="font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: white; border: 1px solid ${data.color}60; padding: 4px 10px; border-radius: 4px;">${b}</span>`).join('')}
            </div>
        </div>
        <div style="position: absolute; top: 0; right: 0; width: 150px; height: 150px; background: radial-gradient(circle, ${data.color}30 0%, transparent 70%); filter: blur(20px); z-index: 1;"></div>
    `;

    const asciiCard = document.createElement("div");
    asciiCard.className = "journey-card journey-card-ascii";

    const asciiContent = document.createElement("div");
    asciiContent.className = "journey-ascii-content";
  ...
  updateCardClipping() {
    ...
    document.querySelectorAll(".journey-card-wrapper").forEach((wrapper) => {
      const rect = wrapper.getBoundingClientRect();
      const cardLeft = rect.left;
      const cardRight = rect.right;
      const cardWidth = rect.width;

      const normalCard = wrapper.querySelector(".journey-card-normal");
      const asciiCard = wrapper.querySelector(".journey-card-ascii");
      ...
      if (cardLeft < scannerRight && cardRight > scannerLeft) {
        ...
        if (!wrapper.hasAttribute("data-scanned") && scannerIntersectLeft > 0) {
          ...
          scanEffect.className = "journey-scan-effect";
  ...
  updateAsciiContent() {
    document.querySelectorAll(".journey-ascii-content").forEach((content) => {
      if (Math.random() < 0.15) {
  ...
  populateCardLine() {
    ...
    const cardsCount = 12; // 3 repetitions of 4 cards
...
// Initialize when the DOM is ready, wait for #journey
setTimeout(() => {
